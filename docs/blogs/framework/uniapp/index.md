...
- 调用 `wx.login()`方法会生成 code ，将 code 作为参数通过 `wx.request()`方法传递给**微信服务器指定接口(后端)**，
- 后端把 `appid,appsecret 和 code` 一起发送到微信服务器。( appid 和 appsecret 都是微信提供的,可以在管理员后台找到)
...
- 小程序端在获取对应的参数后，调用 `wx.requestPavment()`发起微信支付，唤醒支付工作台，进行支付
...
- 而在小程序中，选择了 `Hybrid` 的渲染方式，**将视图层和逻辑层是分开的，双线程同时运行，视图层的界面使用 WebView 进行渲染，逻辑层运行在 JSCore 中**
  - 渲染层:界面渲染相关的任务全都在 `WebView` 线程里执行。一个小程序存在多个界面，所以渲染层**d**
