...
// ===== 引入自定义组件 =====
// 移除静态导入，防止 SSR 阶段触发 @amap/amap-jsapi-loader 的环境检查
// import ZUiComp from '../../../src/index.ts' 
...
export default {
  ...
  enhanceApp({ app, router, siteData }) {
    ...
    app.component('ExampleContainer', ExampleContainer)//自定义示例模板
        // 全局注册组件库
    // app.use(ZUiComp, {})
    // 仅在浏览器端注册组件库，防止 SSR 阶段崩溃
    if (!import.meta.env.SSR) {
      // 使用动态导入，确保在服务端运行期间完全不加载 ZUiComp 及其依赖（如高德地图）
      import('../../../src/index.ts').then((module) => {
        const ZUiComp = module.default
        app.use(ZUiComp, {
          // 组件需要的配置参数名称
          env: {
