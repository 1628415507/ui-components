<template>
  <div class="layout-setting">
    <el-icon size="16" @click="openLayoutSettingDialog"><Setting /></el-icon>
    <!-- 保存布局弹窗-->
    <el-dialog
    ...
    >
      <div v-loading="loading">
        <div class="flex-sc mb-05">
          ...
          <el-button @click="resetDefaultLayout()">{{ $t('vxeTable.resetLayout') }}</el-button>
        </div>
        <div class="flex-sb content-wrap">
          <!-- 右侧布局列表 -->
          <div class="layouts-wrap" v-if="[ROLE_TYPE.TENANT, ROLE_TYPE.USER].includes(roleType)">
          ...
          <el-row :gutter="0" style="height: 100%" class="columns-box">
            ...
            <el-col :span="11" class="columns-wrap show">
              <div class="columns-container">
                <div class="flex-sb columns-container__header">
                  <div class="flex-sc">
                    <el-checkbox
            ...
            <el-col :span="2" class="layout-btn">
              ...
              <el-button @click="moveToLeft" icon="ArrowLeft" />
            </el-col>
            <!-- 隐藏列Hidden Columns -->
            <el-col :span="11" class="columns-wrap">
              <div class="columns-container">
                ...
                <div class="columns-container__content">
                  ...
                  <el-checkbox-group class="columns-list" v-model="hideSelectedKeys" size="large">
                    <draggable
...
import { nextTick, getCurrentInstance, ref, computed, reactive } from 'vue'
...
const showCheckAll = computed({
  ...
  set(val) {
    showSelectedKeys.value = val ? showColumnsList.value.map((it) => it[fieldProp]) : []
  }
})
...
const filterShowColumnsList = computed({
  get() {
    if (!showLabel.value) return showColumnsList.value
    return showColumnsList.value.filter((it) => {
      return it[titleProp].indexOf(showLabel.value) >= 0
    })
  },
  set(val) {
...
const hideCheckAll = computed({
  ...
  set(val) {
    hideSelectedKeys.value = val ? hideColumnsList.value.map((it) => it[fieldProp]) : []
  }
})
...
const filterHideColumnsList = computed({
  get() {
    if (!hideLabel.value) return hideColumnsList.value
    return hideColumnsList.value.filter((it) => {
      return it[titleProp].indexOf(hideLabel.value) >= 0
    })
  },
  set(val) {
...
function loadColumsList() {
  ...
  displayColumns.forEach((showItem, index) => {
    ...
    //   } else {
    //     return showItem[titleProp] == it[titleProp] //没有id就根据标签查找
    //   }
    // })
...
async function getTableData() {
  ...
  if (defaultLayout) {
    ...
    resetParams()
    resetDefaultLayout({ isRender: false }) // 没有已存默认布局时，不调用 renderColumns 以保持原始列顺序
  }
...
//     await proxy.$modal.confirm(proxy.$t('tip.jsIsSaveData'))
//   }
//   done()
// }
...
function moveToLeft() {
  if (!hideSelectedKeys.value.length) {
    ElMessage.error(proxy.$t('tip.checkData'))
    return
  }
  ...
  if (props.limitFieldCount > 0) {
    ...
    if (totalAfterMove > props.limitFieldCount) {
      ...
      ElMessage.error(
        proxy.$t('vxeTable.columnLimitExceeded', {
          limit: props.limitFieldCount,
...
function moveDownItems(selectItems) {
  if (!showSelectedKeys.value.length) {
    return ElMessage.error(proxy.$t('tip.checkData'))
  }
  selectItems.forEach((t) => {
...
function moveUp() {
  ...
  selectedItems.forEach((item, tIndex) => {
...
async function setCurrentItem(item, valid = true) {
  // console.log('【 setCurrentItem 】-585', item, currentItem.value)
  ...
  fullColumn.forEach((col) => {
    const saveItem = layout.find((it) => {
      if (it[fieldProp]) {
        ...
        return col[titleProp] == it.fieldLabel || col[titleProp] == it[titleProp] //没有id就根据标签查找
      }
    })
...
async function initTableTemplate() {
  ...
  if (roleType.value === ROLE_TYPE.TENANT && !layoutTemplates.value.tenantLevelTemplate) {
    // 如果当前租户公司没有租户级模板，则自动创建一条，然后该板不能刑除，不能改名字
