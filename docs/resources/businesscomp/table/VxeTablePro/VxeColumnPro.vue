...
function setFilterOptions(params, isReset = false) {
  ...
  try {
    ...
    const isReset = oldFilterTrigger != resetFiltersTrigger.value
    ...
    if (!isFilter || isReset) {
      ...
      if (optionData.length) {
        ...
        const isFilter = $table.isFilter(colField.value)
        filterOptions.value = options.map((option) => {
          return {
            label: option.label,
            value: option.value,
            tempChecked: isReset && isFilter ? activeFilterValues.includes(option.value) : true
          }
        })
...
function addOutsideClickListener(params: any) {
  ...
  const handleOutsideClick = (event: MouseEvent) => {
    ...
    // 同时检查是否点击在 vxe-table 的筛选图标上（避免关闭）
...
function confirmFilter(params: any) {
  ...
  // if (!currentTableOptionsMap.has(colField.value)) {
  //   currentTableOptionsMap.set(colField.value, getFilteredOptions(params, false))
  // }
  // 移除外部点击监听
  removeOutsideClickListener()
  // 清除状态
