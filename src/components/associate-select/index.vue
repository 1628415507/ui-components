...
<script setup>
  ...
  import service from '@/utils/glpaasService'
  ...
  // 使用数据服务请求
  function serviceRequestData(requestValue) {
    let searchColumns = queryParams.value.requestArg.searchColumns || []
    searchColumns = searchColumns.filter((item) => (requestValue ? item.isFilter !== true : item.isFilter !== true && item.type != 'keyword'))
    // 将keyword类型的值替换成输入框的值
    if (searchColumns) {
    ...
    let requestArg = {
      ...queryParams.value.requestArg,
      pageSize: queryParams.value.pageSize,
      currentPage: queryParams.value.currentPage,
      searchColumns
    }
    let options = queryParams.value.options
    console.log('【 requestArg, param 】-565', requestArg, options)
    return service.request(requestArg, options)
  }
  ...
  function requestData() {
    ...
    } else if (propsConfig.paramType == 'dataService') {
      func = serviceRequestData
    } else {
      func = normalRequest
    }
    loading.value = true
    func(requestValue)
      .then((res) => {
        ...
        loading.value = false
        tableData.value = []
        if (['ilp'].includes(propsConfig.paramType)) {
          tableData.value = res.data.data || []
          totalRows.value = res.data.totalRecord || 0
        } else if (['service'].includes(propsConfig.paramType)) {
          tableData.value = res.data || []
          totalRows.value = res.totalRecord || 0
        } else {
          if (listProp && listProp2) {
            tableData.value = res[listProp][listProp2]
            totalRows.value = res[listProp].total
          } else {
            tableData.value = listProp ? res[listProp] : res.data
            totalRows.value = propsConfig.totalProp ? res[propsConfig.totalProp] : res.total
          }
        }
  ...
  function multiSelectedValue() {
    if (multiple.value && input.value) {
      ...
      multipleOptions.value = clearRepeat(multipleOptions.value.concat(proxy.$refs.associateTable?.getSelectionRows() || []))
    }
  }
  // 可视列的prop
