<template>
  <div class="friendSearchContainer">
    ...
    <el-popover
      ...
      :width="tableWidth"
      :popper-class="`vxe-table--ignore-clear ${popoverClass}`"
      @hide="hide"
    >
      <template #reference>
        <el-input
...
function clickInput() {
  if (props.trigger == 'input') {
    count.value++
    init()
    facPopoverShow.value = !facPopoverShow.value
  }
}
...
function requestData(value) {
  ...
  request({
    ...
    .finally(() => {
      ...
      const focusElementWrapper = inputRef.value?.$el?.querySelector('.el-input__wrapper') //  console.log('【 inputRef 】-399', focusElement == document.activeElement)
      // 解决dev表格无法自动聚焦的问题
...
function clickElse(event, isBlur = false) {
  console.log('【 popoverDom 】-653', popoverDom.value)
  const clickNode = event.target
  console.log('【 clickNode 】-655', clickNode, popoverDom.value?.contains(clickNode))
  if (isBlur) {
...
onMounted(() => {
  calculateTableWidth()
  console.log('【 popoverClass.value 】-782', popoverClass.value)
  document.onclick = clickElse
})
onBeforeMount(() => {
