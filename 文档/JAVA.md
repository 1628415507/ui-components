...
#### 2. 继承的好处
* **提高代码复用性**：可以**把多个子类中重复的代码抽取到父类中**，子类直接继承父类的属性和方法。
...
| `this.name` | 访问本类成员 | 从**本类**（子类）成员变量位置开始往上找 |
| `super.name` | 访问**父类**成员 | 从直接父类成员变量位置开始往上找 |
...
> 利用多态调用方法时，会根据实际传入的子类对象类型，自动调用对应的子类重写方法，
### 10.9.1 什么是多态与表现形式
...
| | **行为动态绑定**<br>*(好处二)* | **如果进行方法重写，利用多态调用方法，可以调用不同子类中重写的方法**。<br>利用多态调用方法时，会根据**实际传入的子类对象类型，自动调用对应的子类重写方法**，实现了业务逻辑与具体实现的解耦。 | - |
...
```java
if (p instanceof Student s) { // 直接声明变量s
...
- 在面向对象的设计中，当多个子类拥有共同的行为，但每个子类的具体实现逻辑完全不同时，我们可以在父类中声明该行为，但无法确定具体的方法体。这种**没有方法体的方法**就是**抽象方法**，
- 而**包含抽象方法的类**必须声明为**抽象类**。
...
| [**匿名内部类**](#10126-匿名内部类核心要点) | 任意可创建对象或作为参数的位置 | **核心重点，必须掌握** | 是后续学习 [**Lambda 表达式**](#1013-lambda-表达式)和**方法引用**的关键基石。 |

---

### 10.12.3 成员内部类核心要点（了解即可）
...
Lambda 表达式是 JDK 8 起引入的新语法，用于简化[函数式接口](#10132-函数式接口)的[匿名内部类](#10126-匿名内部类核心要点)写法。
> 类似前端的匿名函数
### 10.13.1 基本作用与使用前提

| 维度 | 说明 |
| :--- | :--- |
| **基本作用** | 简化函数式接口的匿名内部类写法。 |
| **使用前提** | 必须是接口的匿名内部类，且该接口有且仅有一个抽象方法。 |
| **本质理解** | Lambda 是匿名函数，可理解为一段可传递的代码，写法更简洁、更灵活，提升 Java 语言表达能力。 |
...
| 维度 | 说明 |
| :--- | :--- |
| **定义** | 有且仅有一个抽象方法的[接口](#1011-接口-interface)。 |
| **标记注解** | 可在接口上使用 `@FunctionalInterface` 进行标记；编译器会校验是否满足函数式接口约束。 |
| **与 Lambda 关系** | Lambda **只能**简化函数式接口的匿名内部类，不能用于任意接口或类。 |
...
| 组成部分 | 说明 |
| :--- | :--- |
| **`(参数列表)`** | 对应被简化的抽象方法的形参；类型可写全，也可按省略规则精简。 |
| **`->`** | 分隔参数列表与方法体，读作“指向”。 |
| **`{ 方法体 }`** | 对应被简化的抽象方法的方法体。 |

##### 示例：匿名内部类改为 Lambda（数组排序）
...
匿名内部类中可被去掉的冗余部分通常包括：`new 接口名()`、`@Override`、方法修饰符与方法名；保留参数列表与方法体，用 `->` 连接即可。
...
| 规则 | 说明 |
| :--- | :--- |
| **省略参数类型** | 参数类型可由编译器从函数式接口抽象方法签名推导，可省略不写。 |
| **单个参数省略括号** | 只有一个参数时，参数类型可省略，同时 `()` 也可省略。 |
| **单行方法体同时省略** | 方法体只有一行时，`{}`、`;`、`return` 可省略；三者须同时省略。 |
